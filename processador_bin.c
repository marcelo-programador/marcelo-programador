...
void controle_principal(lista_entradas *l, int *PC)
{
    ...
    for (int i = 0; i < l->quantidade; i++)
    {
        ...
        {
            printf("Erro !!! 1 na definicao de sinais de controle da instrucao %d\n", i + 1);
        ...
        {
            printf("Erro!!! 2 na operacao dos registradores da instrucao %d\n", i + 1);
            continue; // Pula para a próxima instrução
        }
        ...
        {
            printf("Erro!!! 3 na alteracao do pc na instrucao %d\n", i + 1);
            continue; // Pula para a próxima instrução
        }
    }
...
void definir_formato(lista_entradas *l)
{
    for (int i = 0; i < l->quantidade; i++)
    {
        for (int j = 0; j < 7; j++)
        {
            if (l->entradas[i].cod_assembly[j] != '0')
            {
...
bool alteracao_registrador(lista_entradas *l, int i)
{
    ...
    {
        strncpy(rs_bin, l->entradas[i].cod_assembly + 6, 5);
        strncpy(rt_bin, l->entradas[i].cod_assembly + 11, 5);
        strncpy(rd_bin, l->entradas[i].cod_assembly + 16, 5);
        l->entradas[i].rs = strtol(rs_bin, NULL, 2);
        l->entradas[i].rt = strtol(rt_bin, NULL, 2);
        l->entradas[i].rd = strtol(rd_bin, NULL, 2);
        return true;
    }
    ...
    else if (strcmp(l->entradas[i].instrucao_tipo, "lw") == 0 || strcmp(l->entradas[i].instrucao_tipo, "sw") == 0 || strcmp(l->entradas[i].instrucao_tipo, "beq") == 0 || strcmp(l->entradas[i].instrucao_tipo, "j") == 0 || strcmp(l->entradas[i].instrucao_tipo, "addi") == 0)
    {
        strncpy(rs_bin, l->entradas[i].cod_assembly + 6, 5);
        strncpy(rt_bin, l->entradas[i].cod_assembly + 11, 5);
        strncpy(const_or_adress_bin, l->entradas[i].cod_assembly + 16, 16);
        l->entradas[i].rs = strtol(rs_bin, NULL, 2);
        l->entradas[i].rt = strtol(rt_bin, NULL, 2);
        l->entradas[i].const_or_address = strtol(const_or_adress_bin, NULL, 2);
        return true;
    }
